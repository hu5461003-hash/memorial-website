...
-- ============================================================
-- 16. posts 表：公开帖子（任何人匿名可发，私密相册 photo 不公开引用）
-- ============================================================
...
create policy "post_any_insert" on public.posts
  for insert
  with check (
    is_admin = false        -- 匿名发帖入口只允许 is_admin=false，管理员在后台用 admin_write 策略
    and featured = false    -- 匿名不允许设置首页推荐
  );
...
-- ============================================================
-- 17. post_images 表：帖子图片（一帖多图，cover_url 取第一张）
-- ============================================================
...
create index if not exists idx_post_images_order on public.post_images(post_id, sort_order);
...
-- ============================================================
-- 18. post_likes 表：帖子点赞（按 IP 去重，一人一票）
-- ============================================================
...
-- ============================================================
-- 20. posts 存储桶（帖子图片，公开读，管理员上传/删除）
-- ============================================================
insert into storage.buckets (id, name, public) values
  ('posts', 'posts', true)
on conflict (name) do nothing;
...
create policy "posts_bucket_admin_upload" on storage.objects
  for insert
  with check (bucket_id = 'posts' and auth.uid() = 'cb05839f-9e99-4db0-b590-ff0501cb8855');
create policy "posts_bucket_admin_delete" on storage.objects
  for delete
  using (bucket_id = 'posts' and auth.uid() = 'cb05839f-9e99-4db0-b590-ff0501cb8855');
